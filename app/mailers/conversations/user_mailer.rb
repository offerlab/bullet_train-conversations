class Conversations::UserMailer < ApplicationMailer
  helper :theme
  helper "turbo/frames"
  ...
  def notifications(user, conversations_subscription_ids, since)
    ...
    if @conversations_subscriptions.count == 1
      @subject = I18n.t('conversations.user_mailer.notifications.subjects.subject', subject_label: @conversations_subscriptions.first.subject)
    ...
    # if all the conversation threads we're updating them on are on the same team ..
      ...
      @subject = I18n.t('conversations.user_mailer.notifications.subjects.team', team_name: @conversations_subscriptions.first.conversation.team.name)
      mail(to: @user.email, subject: @subject)

    # if the conversation threads we're updating them on are on different teams ..
    else
      @subject = I18n.t('conversations.user_mailer.notifications.subjects.application')
      mail(to: @user.email, subject: @subject)
    end
  end
end
